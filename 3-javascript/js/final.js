...
// 6. Contain( ပါလား မပါဘူးလား), CopyArray, Modified Array, Filter Array
...
// const newProduct = arrayCopy(product);
...
for (product of products) {
  ...
  // saleProducts[saleProductIndex] = product;
  // saleProducts[saleProductIndex].price += 100;
  // saleProductIndex++;
  if (product.price <= 50) {
    saleProducts[saleProductIndex++] = product;
  }
...
const boardSize = 8; // 8x8 checkerboard
...
for (let i = 0; i < boardSize; i++) {
  ...
  for (let j = 0; j < boardSize; j++) {
    // Alternate between 'X' and 'O' characters for squares
    if ((i + j) % 2 === 0) {
      checkerboard += "X";
    } else {
      checkerboard += "O";
...
// Print the checkerboard pattern to the console
...
// 2. studentMark မှဘာသာရပ်တစ်ခုစီ အောင်ရှံးတွက်ပေးပါကြဦး
...
// console.log(student1TotalMark);
// console.log("Overall Result is " + overAllResult);
...
for (student of allStudentsMark) {
  ...
  for (subject of student.marks) {
    if (subject.mark >= 40) {
      ...
      subject.result = student.overAllResult = "Failed";
    }
    // console.log(subject);
  }
